...
bool_t bign96Test()
{
	...
	// управление ключами
	if (bign96KeypairGen(privkey, pubkey, brngCTRXStepR, state) != ERR_OK)
		return FALSE;
	if (!hexEq(privkey,
		...
		return FALSE;
	if (bign96KeypairVal(privkey, pubkey) != ERR_OK)
		return FALSE;
	if (bign96PubkeyVal(pubkey) != ERR_OK)
		return FALSE;
	if (bign96PubkeyCalc(pubkey, privkey) != ERR_OK)
		return FALSE;
	if (!hexEq(pubkey,
		"4CED8FBBA1842BE58B4C0444F359CB14"
		"C6F2CE13B710F1172D2C962F53D13115"
	...
	if (beltHash(hash, beltH(), 13) != ERR_OK)
		return FALSE;
	if (bign96Sign(sig, hash, privkey, brngCTRXStepR, state) != ERR_OK)
		return FALSE;
	if (!hexEq(sig, 
		...
		"E1B2"))
		return FALSE;
	if (bign96Verify(hash, sig, pubkey) != ERR_OK)
		return FALSE;
	sig[0] ^= 1;
	if (bign96Verify(hash, sig, pubkey) == ERR_OK)
		return FALSE;
	...
	if (beltHash(hash, beltH(), 13) != ERR_OK)
		return FALSE;
	if (bign96Sign2(sig, hash, privkey, 0, 0) !=
		ERR_OK)
		return FALSE;
	if (!hexEq(sig, 
