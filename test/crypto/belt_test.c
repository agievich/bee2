...
bool_t beltTest()
{
	...
	beltWBLStart(stack, beltH() + 128, 32);
	beltWBLStepE(buf, 48, stack);
	if (!hexEq(buf,
	...
	beltWBLStart(stack, beltH() + 128 + 32, 32);
	beltWBLStepD(buf, 48, stack);
	if (!hexEq(buf,
	...
	for (count = 32; count <= 128; ++count)
	{
		...
		beltWBLStepE(buf, count, stack);
		memCopy(buf1, buf, count);
		beltWBLStepD(buf, count, stack);
		if (!memEq(buf, beltH(), count))
			return FALSE;
		beltWBLStepD2(buf1, buf1 + count - 16, count, stack);
	...
	memSetZero(hash1, 16);
	beltCompr2((u32*)hash1, (u32*)hash, (u32*)buf, stack);
	...
	beltCompr((u32*)hash1, (u32*)buf, stack);
	u32To(hash1, 32, (u32*)hash1);
	if (!memEq(hash, hash1, 32))
		return FALSE;
	// belt-ecb: тест A.9-1
	memCopy(buf, beltH(), 48);
	beltECBStart(stack, beltH() + 128, 32);
	beltECBStepE(buf, 32, stack);
	beltECBStepE(buf + 32, 48 - 32, stack);
	if (!hexEq(buf,
		"69CCA1C93557C9E3D66BC3E0FA88FA6E"
	...
	memCopy(buf, beltH(), 47);
	beltECBStart(stack, beltH() + 128, 32);
	beltECBStepE(buf, 16, stack);
	beltECBStepE(buf + 16, 47 - 16, stack);
	if (!hexEq(buf,
		"69CCA1C93557C9E3D66BC3E0FA88FA6E"
	...
	memCopy(buf, beltH() + 64, 48);
	beltECBStart(stack, beltH() + 128 + 32, 32);
	beltECBStepD(buf, 16, stack);
	beltECBStepD(buf + 16, 48 - 16, stack);
	if (!hexEq(buf,
		"0DC5300600CAB840B38448E5E993F421"
	...
	beltECBStart(stack, beltH() + 128 + 32, 32);
	beltECBStepD(buf, 16, stack);
	beltECBStepD(buf + 16, 36 - 16, stack);
	if (!hexEq(buf,
		"0DC5300600CAB840B38448E5E993F421"
	...
	memCopy(buf, beltH(), 48);
	beltCBCStart(stack, beltH() + 128, 32, beltH() + 192);
	beltCBCStepE(buf, 32, stack);
	beltCBCStepE(buf + 32, 48 - 32, stack);
	if (!hexEq(buf,
	...
	beltCBCStart(stack, beltH() + 128, 32, beltH() + 192);
	beltCBCStepE(buf, 16, stack);
	beltCBCStepE(buf + 16, 36 - 16, stack);
	if (!hexEq(buf,
		"10116EFAE6AD58EE14852E11DA1B8A74"
	...
	memCopy(buf, beltH() + 64, 48);
	beltCBCStart(stack, beltH() + 128 + 32, 32, beltH() + 192 + 16);
	beltCBCStepD(buf, 16, stack);
	beltCBCStepD(buf + 16, 48 - 16, stack);
	if (!hexEq(buf,
	...
	beltCBCStart(stack, beltH() + 128 + 32, 32, beltH() + 192 + 16);
	beltCBCStepD(buf, 16, stack);
	beltCBCStepD(buf + 16, 36 - 16, stack);
	if (!hexEq(buf,
		"730894D6158E17CC1600185A8F411CAB"
	...
	// belt-cfb: тест A.13
	memCopy(buf, beltH(), 48);
	beltCFBStart(stack, beltH() + 128, 32, beltH() + 192);
	beltCFBStepE(buf, 16, stack);
	beltCFBStepE(buf + 16, 3, stack);
	beltCFBStepE(buf + 16 + 3, 48 - 16 - 3, stack);
	if (!hexEq(buf,
	...
	// belt-cfb: тест A.14
	memCopy(buf, beltH() + 64, 48);
	beltCFBStart(stack, beltH() + 128 + 32, 32, beltH() + 192 + 16);
	beltCFBStepD(buf, 15, stack);
	beltCFBStepD(buf + 15, 7, stack);
	beltCFBStepD(buf + 15 + 7, 48 - 15 - 7, stack);
	if (!hexEq(buf,
	...
	// belt-ctr: тест A.15
	memCopy(buf, beltH(), 48);
	beltCTRStart(stack, beltH() + 128, 32, beltH() + 192);
	beltCTRStepE(buf, 15, stack);
	beltCTRStepE(buf + 15, 7, stack);
	beltCTRStepE(buf + 15 + 7, 48 - 15 - 7, stack);
	if (!hexEq(buf,
	...
	beltCTRStart(stack, beltH() + 128 + 32, 32, beltH() + 192 + 16);
	beltCTRStepD(buf, 11, stack);
	beltCTRStepD(buf + 11, 5, stack);
	beltCTRStepD(buf + 11 + 5, 44 - 11 - 5, stack);
	if (!hexEq(buf,
	...
	beltMACStart(stack, beltH() + 128, 32);
	beltMACStepA(beltH(), 13, stack);
	hexTo(buf, "7260DA60138F96C9");
	if (!beltMACStepV(buf, stack))
		return FALSE;
	...
	beltMACStart(stack, beltH() + 128, 32);
	beltMACStepA(beltH(), 27, stack);
	beltMACStepG(buf, stack);
	beltMACStepA(beltH() + 27, 48 - 27, stack);
	beltMACStepG2(buf, 4, stack);
	hexTo(buf, "2DAB59771B4B16D0");
	if (!beltMACStepV(buf, stack) || !beltMACStepV2(buf, 3, stack))
		return FALSE;
	beltMAC(buf1, beltH(), 48, beltH() + 128, 32);
	if (!memEq(buf, buf1, 8))
		return FALSE;
	// belt-dwp: тест A.19-1 [+ инкрементальность]
	beltDWPStart(stack, beltH() + 128, 32, beltH() + 192);
	memCopy(buf, beltH(), 16);
	beltDWPStepE(buf, 7, stack);
	beltDWPStepE(buf + 7, 9, stack);
	beltDWPStepI(beltH() + 16, 14, stack);
	beltDWPStepG(mac, stack);
	beltDWPStepI(beltH() + 16 + 14, 18, stack);
	beltDWPStepG(mac, stack);
	beltDWPStepA(buf, 12, stack);
	beltDWPStepG(mac, stack);
	beltDWPStepA(buf + 12, 4, stack);
	beltDWPStepG(mac, stack);
	if (!hexEq(buf, 
		"52C9AF96FF50F64435FC43DEF56BD797"))
		return FALSE;
	...
	beltCHEStart(stack, beltH() + 128, 32, beltH() + 192);
	memCopy(buf, beltH(), 15);
	beltCHEStepE(buf, 11, stack);
	beltCHEStepE(buf + 11, 4, stack);
	beltCHEStepI(beltH() + 16, 14, stack);
	beltCHEStepG(mac, stack);
	beltCHEStepI(beltH() + 16 + 14, 18, stack);
	beltCHEStepG(mac, stack);
	beltCHEStepA(buf, 12, stack);
	beltCHEStepG(mac, stack);
	beltCHEStepA(buf + 12, 3, stack);
	beltCHEStepG(mac, stack);
	if (!hexEq(buf,
	...
	beltDWPStart(stack, beltH() + 128 + 32, 32, beltH() + 192 + 16);
	memCopy(buf, beltH() + 64, 16);
	beltDWPStepI(beltH() + 64 + 16, 32, stack);
	beltDWPStepA(buf, 16, stack);
	beltDWPStepD(buf, 16, stack);
	beltDWPStepG(mac, stack);
	if (!hexEq(buf, 
		"DF181ED008A20F43DCBBB93650DAD34B"))
		return FALSE;
	...
	beltCHEStart(stack, beltH() + 128 + 32, 32, beltH() + 192 + 16);
	memCopy(buf, beltH() + 64, 20);
	beltCHEStepI(beltH() + 64 + 16, 32, stack);
	beltCHEStepA(buf, 20, stack);
	beltCHEStepD(buf, 20, stack);
	beltCHEStepG(mac, stack);
	if (!hexEq(buf,
	...
	if (!memEq(buf, buf1, 48))
		return FALSE;
	// belt-kwp: тест A.22
	beltKWPStart(stack, beltH() + 128 + 32, 32);
	memCopy(buf, beltH() + 64, 48);
	beltKWPStepD(buf, 48, stack);
	if (!hexEq(buf,
		"92632EE0C21AD9E09A39343E5C07DAA4"
		"889B03F2E6847EB152EC99F7A4D9F154"))
		return FALSE;
	if (!hexEq(buf + 32, 
		"B5EF68D8E4A39E567153DE13D72254EE"))
		return FALSE;
	if (beltKWPUnwrap(buf1, beltH() + 64, 48, buf + 32,
			beltH() + 128 + 32, 32) != ERR_OK ||
		!memEq(buf, buf1, 32))
		return FALSE;
	// belt-hash: тест A.23-1
	beltHashStart(stack);
	beltHashStepH(beltH(), 13, stack);
	beltHashStepG(hash, stack);
	if (!hexEq(hash,
	...
	beltHashStart(stack);
	beltHashStepH(beltH(), 32, stack);
	hexTo(hash, 
		"749E4C3653AECE5E48DB4761227742EB"
		"6DBE13F4A80F7BEFF1A9CF8D10EE7786");
	if (!beltHashStepV(hash, stack) || !beltHashStepV2(hash, 13, stack))
		return FALSE;
	...
	beltHashStart(stack);
	beltHashStepH(beltH(), 11, stack);
	beltHashStepG2(hash, 32, stack);
	beltHashStepH(beltH() + 11, 48 - 11, stack);
	hexTo(hash, 
		"9D02EE446FB6A29FE5C982D4B13AF9D3"
		"E90861BC4CEF27CF306BFB0B174A154A");
	if (!beltHashStepV2(hash, 32, stack))
		return FALSE;
	...
	memCopy(buf, beltH(), 48);
	beltBDEStart(stack, beltH() + 128, 32, beltH() + 192);
	beltBDEStepE(buf, 32, stack);
	beltBDEStepE(buf + 32, 48 - 32, stack);
	if (!hexEq(buf,
	...
	memCopy(buf, beltH() + 64, 48);
	beltBDEStart(stack, beltH() + 128 + 32, 32, beltH() + 192 + 16);
	beltBDEStepD(buf, 16, stack);
	beltBDEStepD(buf + 16, 48 - 16, stack);
	if (!hexEq(buf,
	...
	memCopy(buf, beltH(), 48);
	beltSDEStart(stack, beltH() + 128, 32);
	beltSDEStepE(buf, 48, beltH() + 192, stack);
	if (!hexEq(buf,
	...
	memCopy(buf, beltH() + 64, 48);
	beltSDEStart(stack, beltH() + 128 + 32, 32);
	beltSDEStepD(buf, 48, beltH() + 192 + 16, stack);
	if (!hexEq(buf,
	...
	memSetZero(level, 12);
	level[0] = 1;
	beltKRPStart(stack, beltH() + 128, 32, level);
	beltKRPStepG(buf, 16, beltH() + 32, stack);
	if (!hexEq(buf, 
	...
	beltKRPStepG(buf, 24, beltH() + 32, stack);
	if (!hexEq(buf,
	...
	beltKRPStepG(buf, 32, beltH() + 32, stack);
	if (!hexEq(buf,
	...
	beltHMACStart(stack, beltH() + 128, 29);
	beltHMACStepA(beltH() + 128 + 64, 32, stack);
	beltHMACStepG(hash, stack);
	if (!hexEq(hash,
	...
	beltHMACStart(stack, beltH() + 128, 32);
	beltHMACStepA(beltH() + 128 + 64, 32, stack);
	hexTo(hash, 
		"41FFE8645AEC0612E952D2CDF8DD508F"
		"3E4A1D9B53F6A1DB293B19FE76B1879F");
	if (!beltHMACStepV(hash, stack))
		return FALSE;
	beltHMAC(hash1, beltH() + 128 + 64, 32, beltH() + 128, 32);
	if (!memEq(hash, hash1, 32))
		return FALSE;
	// belt-hmac: тест Б.1-3 [+ икрементальность]
	beltHMACStart(stack, beltH() + 128, 42);
	beltHMACStepA(beltH() + 128 + 64, 17, stack);
	beltHMACStepG(hash, stack);
	beltHMACStepG2(hash, 17, stack);
	beltHMACStepA(beltH() + 128 + 64 + 17, 32 - 17, stack);
	hexTo(hash, 
		"7D01B84D2315C332277B3653D7EC6470"
		"7EBA7CDFF7FF70077B1DECBD68F2A144");
	if (!beltHMACStepV(hash, stack) || !beltHMACStepV2(hash, 23, stack))
		return FALSE;
