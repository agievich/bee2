...
/*!	\brief Проверка затравочных параметров
	...
	.
	При установке флага repair при обнаружении ошибки в любом из параметров
	zi, di или ri этому параметру присваивается корректное значение по
	умолчанию.
	\return ERR_OK, если параметры корректны (с учетов возможной корректровки
	при repair == TRUE), и код ошибки в противном случае.
*/
err_t stb99ValSeed(
	const stb99_seed* seed	/*!< [in] затравочные параметры */
);

/*!	\brief Заполнение затравочных параметров

	Для затравочных параметров seed проверяется выполнение условий,
	перечисленных в функции stb99ValSeed().
	В случае обнаружения пустых (нулевых) параметров zi, di, ri они
	заполняются -- им назначаются значения по умолчанию:
	-	zi[0] = 1,..., zi[30] = 31;
	...
	в противном случае.
	\remark Параметр l должен быть корректен.
*/
err_t stb99FillSeed(
