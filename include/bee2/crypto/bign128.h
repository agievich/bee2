...
*******************************************************************************
\file bign128.h
\brief Алгоритмы Bign уровня 128
*******************************************************************************
*/
...
Алгоритмы Bign128 -- это алгоритмы Bign со следующими уточнениями:
- уровень стойкости l = 128;
- используется эллиптическая кривая bign-curve256v1;
...
/*!	\brief Генерация пары ключей
	...
	в противном случае.
*/
err_t bign128KeypairGen(
	octet privkey[32],			/*!< [out] личный ключ */
	octet pubkey[64],			/*!< [out] открытый ключ */
...
*/
err_t bign128KeypairVal(
	const octet privkey[32],	/*!< [in] личный ключ */
	const octet pubkey[64]		/*!< [in] открытый ключ */
);
...
*/
err_t bign128PubkeyVal(
	const octet pubkey[64]		/*!< [in] проверяемый ключ */
);
...
err_t bign128DH(
	...
	const octet privkey[32],	/*!< [in] личный ключ */
	const octet pubkey[64],		/*!< [in] открытый ключ (другой стороны) */
...
*/
err_t bign128Sign(
	octet sig[48],				/*!< [out] подпись */
	const octet hash[32],		/*!< [in] хэш-значение */
	const octet privkey[32],	/*!< [in] личный ключ */
	gen_i rng,					/*!< [in] генератор случайных чисел */
	void* rng_state				/*!< [in/out] состояние генератора */
);
...
/*!	\brief Детерминированная выработка ЭЦП
	...
	\expect{ERR_BAD_INPUT} Буферы sig и hash не пересекаются.
	\expect{ERR_BAD_PRIVKEY} Личный ключ privkey корректен.
	\return ERR_OK, если подпись выработана, и код ошибки в противном случае.
*/
err_t bign128Sign2(
	octet sig[48],				/*!< [out] подпись */
	const octet hash[32],		/*!< [in] хэш-значение */
...
/*!	\brief Создание токена ключа
	...
	Создается токен [48 + len]token ключа [len]key с заголовком [16]header. 
	При создании токена используются открытый ключ получателя pubkey 
	и генератор rng с состоянием rng_state.
	\expect{ERR_BAD_INPUT} len >= 16.
	\expect{ERR_BAD_PUBKEY} Открытый ключ pubkey корректен.
	\expect{ERR_BAD_RNG} Генератор rng (с состоянием rng_state) корректен.
	\expect Используется криптографически стойкий генератор rng.
	\return ERR_OK, если токен успешно создан, и код ошибки в противном случае.
	\remark Может передаваться нулевой указатель header. В этом случае будет
	использоваться заголовок из всех нулей.
*/
err_t bign128KeyWrap(
	...
	const octet pubkey[64],		/*!< [in] открытый ключ получателя */
	gen_i rng,					/*!< [in] генератор случайных чисел */
	void* rng_state				/*!< [in,out] состояние генератора */
);
...
/*!	\brief Разбор токена ключа
	...
	Определяется ключ [len - 48]key, который имеет заголовок [16]header 
	и содержится в токене [len]token. При разборе токена используется личный 
	ключ privkey.
	\expect{ERR_BAD_PRIVKEY} Личный ключ privkey корректен.
	...
	Этот код будет возвращен, если len < 64.
*/
err_t bign128KeyUnwrap(
	octet key[],				/*!< [out] ключ */
	const octet token[],		/*!< [in] токен ключа */
	size_t len,					/*!< [in] длина токена в октетах */
	const octet header[16],		/*!< [in] заголовок ключа */
	const octet privkey[32]		/*!< [in] личный ключ получателя */
);
