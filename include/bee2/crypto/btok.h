...
/*!	\brief Проверка CV-сертификата в цепочке
	...
	Проверяется корректность CV-сертификата [cert_len]cert на дату date, 
	для чего используется содержание сертификата издателя cvca. В случае 
	...
	- срока действия, заданный в cvca, корректен;
	- начало действия cert не выходит за пределы срока действия cvca;
	- date попадает в срок действия cert.
	Указатель date может быть нулевым, и тогда последнее условие
	не проверяется. Указатель cvc может быть нулевым, и тогда содержание
