...
*******************************************************************************
\file bign256.h
\brief Алгоритмы Bign уровня 256
*******************************************************************************
*/
...
Алгоритмы bign256 -- это алгоритмы Bign со следующими уточнениями:
- уровень стойкости l = 256;
- используется эллиптическая кривая bign-curve512v1;
- при выработке и проверке ЭЦП предварительное хэширование выполняется с 
  помощью алгоритма bash512.
...
/*!	\brief Генерация пары ключей
	...
	в противном случае.
*/
err_t bign256KeypairGen(
	octet privkey[64],			/*!< [out] личный ключ */
	octet pubkey[128],			/*!< [out] открытый ключ */
...
*/
err_t bign256KeypairVal(
	const octet privkey[64],	/*!< [in] личный ключ */
	const octet pubkey[128]		/*!< [in] открытый ключ */
);
...
err_t bign256DH(
	...
	const octet privkey[64],	/*!< [in] личный ключ */
	const octet pubkey[128],	/*!< [in] открытый ключ (другой стороны) */
...
*/
err_t bign256Sign(
	octet sig[96],				/*!< [out] подпись */
	const octet hash[64],		/*!< [in] хэш-значение */
	const octet privkey[64],	/*!< [in] личный ключ */
	gen_i rng,					/*!< [in] генератор случайных чисел */
	void* rng_state				/*!< [in/out] состояние генератора */
);
...
err_t bign256KeyWrap(
	...
	const octet pubkey[128],	/*!< [in] открытый ключ получателя */
	gen_i rng,					/*!< [in] генератор случайных чисел */
	void* rng_state				/*!< [in,out] состояние генератора */
);
...
/*!	\brief Разбор токена ключа
	...
	Определяется ключ [len - 80]key, который имеет заголовок [16]header 
	...
	Этот код будет возвращен, если len < 96.
*/
err_t bign256KeyUnwrap(
	octet key[],				/*!< [out] ключ */
	const octet token[],		/*!< [in] токен ключа */
	size_t len,					/*!< [in] длина токена в октетах */
	const octet header[16],		/*!< [in] заголовок ключа */
	const octet privkey[64]		/*!< [in] личный ключ получателя */
);
