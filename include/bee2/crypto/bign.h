...
/*!	\brief Генерация долговременных параметров
	...
	params->seed увеличивается на 1 (как число little-endian) по модулю 2^64, 
	вычисление и проверка параметров продолжаются. Параметр params->q 
	вычисляется с помощью функции calc_q(). Функция on_seed() получает 
	управление всякий раз перед обработкой очередного seed. Функции calc_q() 
	и on_seed() используют в своей работе состояние state.
	...
	\remark Если возвращаемый функциями calc_q() и on_seed() код отличен 
	от ERR_OK, то вычисления завершаются с возвратом этого кода.
	Исключение составляет случай, когда функция calc_q() возвращает код
	ERR_NO_RESULT. В этом случае выполняется переход к следующему seed и 
