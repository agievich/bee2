...
/*!	\brief Проверка очередного пароля в режиме HOTP
	...
	дополнительных попыток. Если совпадение не найдено, то счетчик будет 
	возвращен к первоначальному значению. 
	\pre attempts < 10.
...
При проверке пароля будут просмотрены корректные (без переполнения tm_time_t) 
отметки в окрестности [t - attempts_bwd, t + attempts_fwd] округленной 
отметки t.  Ограничения на attempts_bwd / attempts_fwd введены для защиты 
от зацикливания и для контроля вероятности угадывания пароля.
*******************************************************************************
*/
...
/*!	\brief Проверка очередного пароля в режиме TOTP
	...
	Полученный пароль сравнивается с otp. Если пароли различаются, то процедура
	повторяется с другими корректными отметками времени из интервала 
	[t - attempts_bwd, t + attempts_fwd] до тех пор, пока совпадение паролей 
	не будет обнаружено или весь интервал не будет просмотрен.
...
/*!	\brief Проверка пароля в режиме TOTP
	...
	с другими корректными отметками времени из интервала 
	[t - attempts_bwd, t + attempts_fwd] до тех пор, пока совпадение паролей 
	не будет обнаружено или весь интервал не будет просмотрен.
	\expect{ERR_BAD_PWD} 6 <= digit && digit <= 8.
	\expect{ERR_BAD_TIME} t != TIME_ERR.
...
(шестнадцатеричный). Для предварительной проверки запроса можно использовать 
функции strIsAlphanumeric(), decIsValid(), hexIsValid().
*******************************************************************************
*/
...
/*!	\brief Генерация очередного пароля в режиме OCRA
	...
	По запросу q и округленной отметке t текущего времени, по описателю suite,
	ключу и данным сеанса, размещенным в state, генерируется одноразовый 
	пароль otp. Длины otp и q определяются описателем suite. Если suite задает 
	использование счетчика, то после генерации пароля он инкрементируется.
...
/*!	\brief Проверка очередного пароля в режиме OCRA
	...
	По запросу q и округленной отметке t текущего времени, по описателю suite, 
	ключу и данным сеанса, размещенным в state, строится одноразовый пароль 
	otp. Длина q определяется описателем suite. Если suite задает использование
	счетчика, то после успешной проверки пароля он инкрементируется.
	\pre Если suite задает использование t, то t != TIME_ERR.
	\expect Формат q соответствует типу, указанному в suite.
...
/*!	\brief Генерация пароля в режиме OCRA
	...
	отметке t текущего времени генерируется одноразовый пароль otp. Если suite 
	задает использование счетчика, то после генерации пароля 
	он инкрементируется.
	\expect{ERR_BAD_PARAMS} Формат suite корректен.
...
/*!	\brief Проверка пароля в режиме TOTP
	...
	Построенный пароль сравнивается с otp. Если suite задает использование 
	счетчика, то после успешной проверки пароля ctr инкрементируется.
	\expect{ERR_BAD_PARAMS} Формат suite корректен.
	\expect{ERR_BAD_TIME} Если suite задает использование t, то t != TIME_ERR.
	\expect{ERR_BAD_PWD} Пароль otp подошел.
	\expect Формат q соответствует типу, указанному в suite.
