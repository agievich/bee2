...
блока (Block) и шифрования широкого блока (WBL). Механизм WBL определен 
в новой редакции СТБ 34.101.31 и представляет собой ядро KWP.

В механизмах ECB, CBC, CFB, CTR, MAC, DWP, KWP, BDE, FMT, KRP используется 
ключ из 32 октетов. Ключ такой длины может быть построен по ключу из 16
...
В механизмах ECB, CBC, CFB, CTR, MAC, DWP, BDE, KWP данные обрабатываются 
...
/*!	\brief Зашифрование в режиме WBL
	...
	\remark Функцию beltWBLStepE() можно применять многократно. 
	Зашифрование в режиме WBL можно применять многократно. При первом
	обращении счетчик i меняется от 1 до 2n, при втором --- от 2n + 1 до 4n,
	и так далее. Многократное зашифрование в режиме WBL используется для генерации 
	одноразового ключа в СТБ 34.101.45. 
	\remark Расшифрование в режиме WBL корректно работает, только если
...
/*!	\brief Расшифрование в режиме WBL
	...
	\remark Сохраняются замечания к функции beltWBLStepE() с точностью до
...
/*
*******************************************************************************
Шифрование и имитозащита данных (belt-dwp, DWP)
*******************************************************************************
*/
...
/*!	\brief Инициализация функций KWP
	...
	По ключу [len]key в state формируются структуры данных,
	необходимые для работы в режиме KWP.
	\pre len == 16 || len == 24 || len == 32.
...
/*!	\brief Зашифрование в режиме KWP
	...
	Буфер [count]buf, имеющий вид [count - 16]key_to_wrap || [16]header, 
	зашифровывается в режиме WBL на ключе, размещенном в state. Результат 
	зашифрования сохраняется в [count]buf.
	\pre count >= 32.
	\expect beltKWPStart() < beltKWPStepE()*.
*/
void beltKWPStepE(
	void* buf,				/*!< [in/out] ключ + заголовок / зашифр. ключ */
	size_t count,			/*!< [in] длина ключа + заголовка / зашиф. ключа */
	void* state				/*!< [in/out] состояние */
);

/*!	\brief Расшифрование в режиме KWP

	Зашифрованный ключ [count]buf расшифровывается в режиме WBL на ключе, 
	размещенном в state. Результат зашифрования сохраняется в [count]buf. 
	Результат имеет вид [count - 16]unwrapped_key || [16]header, 
	\pre count >= 32.
	\expect beltKWPStart() < beltKWPStepD()*.
*/
void beltKWPStepD(
	void* buf,				/*!< [in/out] зашифр. ключ / ключ + заголовок */
	size_t count,			/*!< [in] длина зашиф. ключа / ключа + заголовка  */
	void* state				/*!< [in/out] состояние */
);

/*!	\brief Расшифрование в режиме KWP с выделением заголовка
...
*/
void beltKWPStepD2(
	void* buf1,		/*!< [in/out] первая часть заш. ключа / открытый ключ */
	void* buf2,		/*!< [in/out] вторая часть заш. ключа / заголовок ключа */
	size_t count,	/*!< [in] длина зашифрованного ключа */
	void* state		/*!< [in/out] состояние */
);
