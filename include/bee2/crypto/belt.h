...
В механизмах ECB, CBC, CFB, CTR, MAC, DWP, KWP, BDE, SDE, FMT, KRP используется 
...
В механизмах ECB, CBC, CFB, CTR, MAC, DWP, KWP, BDE данные обрабатываются 
...
/*!	\brief Длина состояния функций сжатия

	Возвращается длина состояния (в октетах) функций сжатия.
	\return Длина состояния.
*/
...
/*!	\brief Сжатие форматированных данных
	...
	Форматированный буфер h || buf сжимается до форматированного буфера h.
	\pre Буферы buf и h не пересекаются.
	\deep{stack} beltCompr_deep().
*/
void beltCompr(
	u32 h[8],			/*!< [in/out] первая часть входа/выход */
	const u32 buf[8],	/*!< [in] вторая часть входа */
...
/*!	\brief Сжатие форматированных данных со сложением
	...
	Форматированный буфер h || buf сжимается до форматированного буфера h.
	Внутренняя переменная S добавдяется поразрядно по модулю 2 к буферу s.
	\pre Буферы s и h, buf и s, buf и h не пересекаются.
	\deep{stack} beltCompr_deep().
*/
void beltCompr2(
