...
/*!	\brief Инициализация режима HMAC
	...
	повторно сгенерированы те же данные.
 	\warning Если размер синхропосылки (iv_len) больше чем 64 байта, то
 	синхропосылка не копируется внутрь состояния генератора, и указатель
 	на синхропосылку iv должен указывать на область памяти не освобождаемую в течении
 	всего времени работы с генератором.
