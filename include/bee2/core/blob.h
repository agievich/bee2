...
/*!	\brief Разметка блоба
	...
	Действуют правила функции memSlice(). В частности, 
	- указатели pi выровнены на границу фундаментального блока;
	- при установке в ci флага SIZE_HI указатель pi повторяет предыдущий, если
	  таковой имеется.
	Если памяти для размещения фрагментов недостаточно, то размер блоба 
	увеличивается.
	\return Дескриптор блоба. Нулевой дескриптор возвращается при 
	нехватке памяти.
	\post Указатели pi, если они формируются, выровнены на границу 
	фундаментального блока. 
	\remark Разрешается передавать нулевой дескриптор blob. В этом случае блоб 
	будет создан.
*/
blob_t blobSlice(
	blob_t blob,	/*!< [in] блоб */
