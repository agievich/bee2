...
/*!	\brief Создание блоба с разметкой

	Создается блоб для размещения фрагментов памяти [s1]p1, [s2]p2, ...., 
	[sn]pn. Фрагменты описываются параметрами s1 и .... Описание имеет вид:
		s1, pp1, s2, pp2, ..., sn, ppn, SIZE_MAX,
	где si -- длина фрагмента (size_t), ppi -- указатель на pi (const void**),
	SIZE_MAX -- маркер окончания списка параметров. Действуют правила функции
	memSlice(). В частности, 
	- указатели pi выровнены на границу фундаментального блока;
	- при установке в si флага SIZE_HI указатель pi повторяет предыдущий, если
	  таковой имеется.
	\return Дескриптор созданного блоба. Нулевой дескриптор возвращается при 
	нехватке памяти.
	\post Выходной блоб корректен.
	\remark При создании блоба все его октеты обнуляются.
*/
blob_t blobCreate2(
	size_t s1,	/*!< [in] размер первого фрагмента */
