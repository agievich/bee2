...
/*!	\brief Длина фундаментального блока

	Определяется длина фундаментального блока.
	\return Длина фундаментального блока.
*/
size_t memMaxAlign();
...
/*!	\brief Выделение блока памяти
	...
	блока: memIsAligned(p, memMaxAlign()) == TRUE.
	\remark Разрешается вызов memAlloc(0). Результатом вызова может быть 
	нулевой указатель. Этот указатель можно корректно передать в memRealloc() 
	и memFree().
*/
void* memAlloc(
...
/*!	\brief Разметка буфера памяти
	...
	описываются параметрами .... Описание имеет вид:
		с1, pp1, с2, pp2, ..., сn, ppn, SIZE_MAX,
	где сi -- длина фрагмента в октетах (size_t), ppi -- указатель на pi 
	(const void**), SIZE_MAX -- маркер окончания списка параметров. 
	...
	2. Разрешается передавать отдельный нулевой указатель ppi. В этом случае 
 	...
 	3. При установке в сi флага SIZE_HI указатель pi повторяет предыдущий, если 
...
size_t memSlice(
	const void* buf,	/*!< [in] буфер */
	...					/*!< [in,out] описание фрагментов памяти */
);

/*!	\brief Разметка буфера памяти 2

	Буфер buf разбивается на фрагменты, описанные параметрами args. 
	Описание имеет вид:
		c1, pp1, c2, pp2, ..., cn, ppn, SIZE_MAX.
	Действуют соглашения memSlice().
	\pre Указатель buf выровнен на границу фундаментального блока.
	\post Указатели pi, если они формируются, выровнены на границу 
	фундаментального блока: memIsAligned(pi, memMaxAlign()) == TRUE.
	\return Суммарная длина фрагментов с учетом выравнивания и возможных 
	пересечений.
*/
size_t memSlice2(
	const void* buf,	/*!< [in] буфер */
	va_list	args		/*!< [in,out] описание фрагментов памяти */
);
