...
/*!	\brief Разметка буфера памяти
	...
	Буфер buf разбивается на фрагменты, описанные аргументами ....
	Описание имеет вид:
		s1 p1 s2 p2 ... sn pn s_{n+1}.
	Здесь si -- аргумент типа size_t, pi -- аргумент типа const void**.
	Аргумент s_{n+1} равняется SIZE_MAX, данное значение является признаком
	окончания списка аргументов. Пара (si, pi) описывает i-й фрагмент: 
	- si -- длина фрагмента,
	- pi -- указатель на указатель на начало фрагмента.
	Дополнительные правила:
	1. Разрешается передавать нулевой указатель buf. В этом случае указатели на 
       начала фрагментов не формируются. 
	2. Разрешается передавать нулевой указатель pi. В этом случае указатель на
	   начало i-го фрагмента не формируется.
	3. Если указатель на начало фрагмента сформирован, то он обязательно 
	   выровнен так, чтобы фрагмент годился для размещения любого объекта.
	4. При установке в si флага SIZE_HI указатель на i-й фрагмент повторяет 
	   предыдущий, если таковой имеется. Это повзоляет использовать один и тот 
	   же участок памяти в разные моменты времени для размещения разных 
	   объектов.
	\return Суммарная длина фрагментов (с учетом выравнивания и пересечений).
	\remark Дополнительное правило 3 означает, что адрес начала фрагмента 
	кратен размеру любого скалярного типа, использууемого в библиотеке.
*/
size_t memSlice(
	const void* buf,	/*!< [in] буфер */
	...					/*!< [in,out] аргументы */
