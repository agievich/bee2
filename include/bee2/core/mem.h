...
/*!	\brief Разметка буфера памяти
	...
	Буфер buf разбивается на фрагменты [s1]p1, ... [sn]pn. Фрагменты 
	описываются параметрами s1 и .... Описание имеет вид:
		s1, pp1, s2, pp2, ..., sn, ppn, SIZE_MAX,
	где si -- длина фрагмента (size_t), ppi -- указатель на pi (const void**), 
	SIZE_MAX -- маркер окончания списка параметров. 
	Дополнительные правила:
	1. Разрешается передавать нулевой указатель buf. В этом случае ни один из 
	   указателей pi не формируются. Определяется только суммарная длина 
	   фрагментов.
	2. Разрешается передавать отдельный нулевой указатель ppi. В этом случае 
	   указатель pi не формируется.
 	3. При установке в si флага SIZE_HI указатель pi повторяет предыдущий, если 
	...
	пересечений.
	\remark Фундаментальный блок инкапсулирует все скалярные типы Bee2. По 
	адресу, выровненному на границу блока, можно размещать любой скалярный 
	тип и, следовательно, любой объект.
*/
size_t memSlice(
	const void* buf,	/*!< [in] буфер */
	size_t s1,			/*!< [in] длина первого фрагмента */
	...					/*!< [in,out] остальные параметры */
);

/*!	\brief Разметка буфера памяти 2

	Буфер buf разбивается на фрагменты, описанные параметрами s1 и args. 
	Описание имеет вид:
		s1, pp1, s2, pp2, ..., sn, ppn, SIZE_MAX.
...
size_t memSlice2(
	const void* buf,	/*!< [in] буфер */
	size_t s1,			/*!< [in] длина первого фрагмента */
	va_list	args		/*!< [in,out] остальные параметры */
);
