...
/*!	\brief Разметка буфера памяти
	...
	1. Разрешается передавать нулевой указатель buf. В этом случае определяется
	   только суммарная длина фрагментову. Указатели ppi не используются и их 
	   можно не передавать.
	...
	\pre Если buf != 0, то указатели ppi корректны.
	\post Если buf != 0, то указатели pi выровнены на границу фундаментального 
	блока: memIsAligned(pi, sizeof(mem_block_t)) == TRUE.
	...
	\remark Пример работы с управляемым стеком с помощью memSlice():
	\code
		void f(..., void* stack)
		{
			octet* p1;
	...
	\endcode 
	Функция memSlice() изменяет указатель stack. Выравнивание указателя на 
	границу фундаментального блока сохраняется.
*/
size_t memSlice(
	const void* buf,	/*!< [in] буфер */
...
/*!	\brief Разметка буфера памяти 2
	...
	Буфер buf разбивается на фрагменты [c1]p1, ..., [cn]pn, описанные 
	параметрами c1 и args. Описание имеет вид:
		c1, c2, ..., cn, SIZE_MAX, pp1, pp2, ..., ppn.
...
size_t memSlice2(
	const void* buf,	/*!< [in] буфер */
	size_t c1,			/*!< [in] начало описания фрагментов памяти */
	va_list	args		/*!< [in,out] окончание описания */
);
