...
По адресу ec->pre могут размещаться предвычисленные точки, построеннные 
...
в группе точек этой кривой. Функции интерфейсов ec_dbladd_i, ec_dbladda_i,
ec_tpl_i, ec_finadd_i, ec_finadda_i и ec_smul_i можно не поддерживать.
...
#define ecDblAdd(c, a, b, ec, stack)\
	((ec)->dbladd ?\
		(ec)->dbladd(c, a, b, ec, stack) :\
		(ecDbl(c, a, ec, stack), ecAdd(c, c, b, ec, stack)))
