...
/*!	\brief Кратная точка по методу SO
	...
	-	функция (ec->finadd != 0) ? ec->finadd : ec->add регулярна: сложения 
		a + b, a + a, a + (-a) выполняются по одним и тем же формулам без 
		условных переходов; 
	- 	преобразование в аффинную точку в функции (ec->finadd != 0) ? 
		ec->finadd : ec->toa выполняется без условных переходов.
*/
bool_t ecMulPreSO(
	word b[],				/*!< [out] кратная точка */
	const ec_pre_t* pre,	/*!< [in] предвычисленные точки */
...
/*!	\brief Кратная точка по методу SOA
	...
	-	функция (ec->finadda != 0) ? ec->finadda : ec->adda регулярна: сложения
		a + b, a + a, a + (-a) выполняются по одним и тем же формулам без 
		условных переходов; 
	- 	преобразование в аффинную точку в функции (ec->finadda != 0) ? 
		ec->finadda : ec->toa выполняется без условных переходов.
*/
bool_t ecMulPreSOA(
	word b[],				/*!< [out] кратная точка */
	const ec_pre_t* pre,	/*!< [in] предвычисленные точки */
