...
/*!	\brief Предвычисленные точки 
	...
	- Comb: вычисляются аффинные точки
		\pm a + \pm h a + \pm h^2 a + ... + \pm h^{w-1} a.
	.
	\pre 0 < w && w < B_PER_W.
...
/*!	\brief Кратная точка с регуляризацией
	...
	-	ec->pmaadd != 0 и функция ec->pmaadd регулярна: сложение и удвоение
		выполняются по одним и тем же формулам без условных переходов;
	-	функция ec->dbl регулярна: удвоение точек, отличных от O, выполняется
		без условных переходов;
	-	функция ec->add (ec->dbladd == 0) или ec->dbladd (ec->dbladd != 0)
		регулярна: сложение различных и не взаимно обратных точек выполняется
		без условных переходов.
*/
bool_t ecMulA2(
	word b[],			/*!< [out] кратная точка */
