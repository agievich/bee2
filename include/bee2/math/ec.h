...
в группе точек этой кривой. Функции интерфейсов ec_dbladda_i, ec_tpl_i,
ec_finadd_i и ec_finadda_i можно не поддерживать. Указатель на
неподдерживаемую функцию должен быть нулевым.
...
ecGroupCreate() или подобных функций. 
...
typedef enum
{
	...
	ec_pre_comb, 		/*!< схема Comb */
} ec_pre_type;

/*!	\brief Предвычисленные точки 

	В поле pts размещаются точки эллиптической кривой, построенные 
	по некоторой точке a в соответствии с определенной схемой 
	предвычислений. Число предвычисленных точек определяется
	шириной окна w и равняется 2^w. Поддерживаются следующие схемы:
	- SNZ (Signed Non-Zero): вычисляется проективные точки
		a, 3a, ..., (2^w-1)a, -(2^w-1)a, ..., -3a, -a;
	- SNZA: вычисляются аффинные точки
		a, 3a, ..., (2^w-1)a, -(2^w-1)a, ..., -3a, -a;
	- Comb: вычисляются аффинные точки
		[b_{w-1} ... b_1 b_0]a = b_{w-1} h^{w-1} a + ... + b_1 h a + b_0 a,
	  где b_i in {-1, 1}. Точка [b_{w-1} ... b_1 b_0]a размещаются
	  в массиве pts по индексу, двоичная запись которого есть
		с_{w-1}...c_1 c_0,
	  где c_i = 1, если b_i = 1, и c_i = 0, если b_i = -1.
	.
	\pre 0 < w && w < B_PER_W.
