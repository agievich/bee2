...
#define wwH16(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p)\
	wwH8(a,b,c,d,e,f,g,h), wwH8(i,j,k,l,m,n,o,p)
...
/*!	\brief Выбор фрагмента

	В слове [count * m]a, которое интерпретируется как объединение count
	фрагментов a[0], a[1], ..., a[n - 1] по m слов в каждом, выбирается
	и размещается в буфере [m]b фрагмент под номером pos:
		b <- a[pos].
	\pre Буферы a и b не пересекаются.
	\remark Функция используется при регуляризации.
*/
void wwSel(
	word b[],		/*!< [out] фрагмент */
	const word a[],	/*!< [in] слово */
	size_t count,	/*!< [in] число фрагментов */
	size_t m,		/*!< [in] длина фрагмента в машинных словах */
	size_t pos		/*!< [in] номер фрагмента */
);
...
/*!	\brief Расчет NAF
	...
	В [2n + 1]naf формируется оконная оптимальная знаковая форма (Non-Adjacent
	Form, NAF) слова [n]a. NAF представляет собой последовательность цифр
	(a_{l-1}, ..., a_1, a_0) такую, что
	-	a_i \in {0, \pm 1, \pm 3, ..., \pm 2^{w-1} - 1};
	-	если a != 0, то a_{l-1} != 0;
	-	a как число равняется \sum {i=0}^{l-1} a_i 2^i;
	-	среди любых w последовательных цифр a_i только одна ненулевая.
	.
	Подробнее см. [Hankerson D., Menezes A., Vanstone S. Guide to Elliptic
	Curve Cryptography, Springer, 2004].
	Нулевая цифра a_i кодируется единственным нулевым битом. Ненулевая цифра
	кодирируется w битами стандартного двоичного представления. Например,
	цифра -1 кодируется битами 11...11, цифра -(2^{w-1} - 1) -- битами 10..01.
	Код a_{l-1} записывается в младшие биты naf, код a_0 -- в старшие.
	\pre 2 <= w < B_PER_W.
	\pre Буфер naf не пересекается с буфером a.
	\return Длина naf (число цифр l).
	\safe Функция нерегулярна.
	\remark Длина NAF не превосходит wwBitSize(a) + 1.
	\remark Для кодирования w последовательных цифр потребуется не
	более (w - 1) * 1 + 1 * w = 2 * w - 1 битов. Поэтому при w < B_PER_W для
	хранения всего кодового представления потребуется не более 2 * n + 1 слов.
	\remark Декодирование NAF выполняется с помощью окна длины w, которое
	смещается от младших битов naf к старшим по следующим правилам:
	-	если слово в окне заканчивается нулевым битом, то декодируется цифра 0
		и окно смещается на 1 позицию;
	-	в зависимости от того, начинается слово в окне нулевым или единичным
		битом, декодируется положительная или отрицательная цифра и окно
		смещается на w позиций.
	.
*/
size_t wwNAF(
