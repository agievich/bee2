...
/*!	\brief Расчет NAF
	...
	В [2n + 1]naf помещается кодированное представление оконной оптимальной
	знаковой формы (NAF) слова [n]a. NAF представляет собой последовательность
	цифр (a_0, a_1,..., a_{l-1}) такую, что
	...
	\remark Кодирование цифр NAF состоит в следующем (\bin(b) -- двоичная
	запись числа b):
	-	нулевые a_i представляются одним двоичным символом 0;
	-	положительные a_i представляются w битами 0\bin(a_i);
	-	отрицательные a_i представляются w битами 1\bin(|a_i|);
	-	кодированное представление - это конкатенация кода a_{l-1}
		(первые биты), ...., кода a_1, кода a_0 (последние биты).
	.
	...
	\remark Декодирование NAF выполняется с помощью окна длины w:
	-	если слово в окне заканчивается нулевым битом, то декодируется цифра 0
		и окно смещается на 1 позицию вправо;
	-	в зависимости от того, начинается слово в окне нулевым или единичным
		...
		смещается на w позиций вправо.
	.
	\remark Суффикс старших разрядов (\alpha, 0,..., 0, 1), в котором w - 1 нулей
	и цифра \alpha < 0, заменяется на суффикс (\beta, 0,..., 0, 1), в котором
	w - 2 нулей и цифра \beta = 2^{w - 1} + \alpha > 0. Замена корректна, так как
	суффиксы описывают одинаковые числа:
		2^w + \alpha = 2^{w - 1} + \beta.
	При этом длина второго суффикса и соответствующей NAF-формы на единицу меньше.
*/
size_t wwNAF(
