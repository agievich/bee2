...
/*!	\brief Создание эллиптической кривой в якобиевых координатах
	...
	Создается описание ec эллиптической кривой в якобиевых координатах над
	полем f с коэффициентами [f->no]A и [f->no]B. При формировании описания
	учитывается флаг odd_order, в котором указывается признак нечетности
	порядка группы точек кривой.
	...
	\remark Передача odd_order = FALSE означает отсутствие гарантий нечетности
	порядка и не означает, что порядок обязательно является нечетным.
*/
bool_t ecpCreateJ(
	...
	bool_t odd_order,	/*!< [in] признак нечетности порядка группы */
