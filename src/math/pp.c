...
\todo Протестировать 16-разрядный случай.

\todo Протестировать 64-разрядный случай.
*******************************************************************************
*/
	...
	void* stack)
{
	...
	else if ((n & 1) == 0)
	{
		...
		ppMulEq(c, a, b, m, w);
		// c3 || c2 <- a1 b1
		ppMulEq(c + 2 * m, a + m, b + m, m, w);
		// w0 <- a0 + a1, w1 <- b0 + b1, w2 <- c1 + c2
		for (i = 0; i < m; ++i)
			w[i] = a[i] ^ a[m + i],
			w[m + i] = b[i] ^ b[m + i],
			w[2 * m + i] = c[m + i] ^ c[2 * m + i];
		// c2 || c1 <- w0 w1
		ppMulEq(c + m, w, w + m, m, w + 3 * m);
		// c1 <- c1 + c0 + w2, c2 <- c2 + c3 + w2
		for (i = 0; i < m; ++i)
			c[m + i] ^= c[i] ^ w[2 * m + i],
			c[2 * m + i] ^= c[3 * m + i] ^ w[2 * m + i];
	}
