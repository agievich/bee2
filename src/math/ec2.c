...
Используются проективные координаты Лопеса -- Дахаба (LD, [LopDah98]):
	x = X / Z, y = Y / Z^2,
	O = (1 : 0 : 0),
	-(X : Y : Z) = (X : ZX + Y : Z).
...
на платформе x86 и возможно требует пересмотра. На сайте
http://www.hyperelliptic.org/efd используется другое соотношение: 1D = 10M.
Считаем его слишком оптимистичным (по отношению к D).
