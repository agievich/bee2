...
Реализовано предложение [Wal13]: наблюдением является разность между
показаниями высокоточного таймера (например, регистра RDTSC) при приостановке
потока на 0 мс, т.е. при передаче управления ядру.

Оценка энтропии при использовании RDTSC [Wal13]: 4.9 битов / наблюдение.
...
[Wal13] Walker J. Seeding Random NumberGenerators. Workshop on Real-World
        Cryptography, Stanford University, Jan., 9-11, 2013,
        https://crypto.stanford.edu/RealWorldCrypto/slides/jesse.pdf.

\warning [Wal13]: наблюдения зависимы, модель AR(1).
...
Реализована идея работ [Mul22, SezSen03]: наблюдением является разность между
показаниями таймера-счетчика до и после выполнения определенного набора
инструкций процессора. Время выполнения набора предположительно флуктуирует.
...
-	отсчеты времени обрабатываются LFSR с характеристическим многочленом
	...
	(в соответствии с [KhaAgi01; стр. 155]);
...
[Mul22]    Muller S. CPU Time Jitter Based Non-Physical True Random Number
           Generator, 2014-2022, 
		   https://www.chronox.de/jent/CPU-Jitter-NPTRNG-v2.2.0.pdf.
[SezSen03] Sezner A., Sendrier N. HAVEGE: A user-level software heuristic
           for generating empirically strong random numbers. ACM Transactions
		   on Modeling and Computer Simulation (TOMACS), 13 (4), p. 334-346,
		   https://doi.org/10.1145/945511.945516.
[KhaAgi01] Харин Ю.С., Агиевич С.В. Компьютерный практикум по математическим
		   методам защиты информации. Мн.: БГУ, 2001.

\remark Дополнительные ссылки / информация / реализации:
