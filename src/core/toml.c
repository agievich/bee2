...
Детали реализации:
* Функция tomlSpaceDec() возвращает число пробелов вплоть до первого 
  непробела или символа '\0', указывающего на конец обрабатываемой строки 
  (что встретится первым). Возвращает 0, если пробелов нет. Никогда не 
  возвращает SIZE_MAX.
* Функция tomlLFDec() возвращает число символов вплоть до первого непробела 
  после LF = '\n' или CRLF = "\r\n" или символа '\0' (что встретится первым).
* Функция tomlCommentDec() возвращает число символов вплоть до LF или CRLF
  или '\0' (что встретится первым). Возвращает 0, если комментарий
...
* Функция tomlVertDec() возвращает число символов вплоть до первого непробела
  после вертикального блока из комментариев и пустых строк (завершаются LF 
  или CRLF). Возвращает SIZE_MAX, если блок содержит комментарии и в них
...
static size_t tomlCommentDec(const char* toml)
{
	...
	while (1)
	{
		...
		while (toml[0] && toml[0] != '\n' && 
			(toml[0] != '\r' || toml[0] != '\n'))
		{
			// управляющий символ, отличный от '\t'?
			if (0x00 <= toml[0] && toml[0] <= 0x1F && toml[0] != '\t' ||
				toml[0] == 0x7F)
				return SIZE_MAX;
			// к следующему символу
			++count, ++toml;
		}
		// комментарий продолжается на следующей строке?
		c = tomlLFDec(toml);
		ASSERT(c != SIZE_MAX);
		if (toml[c] != '#')
			break;
...
2. Разрывов может быть несколько. Разрывы могут устанавливаться только после 
   ...
   разрывами отсутствуют шестнадцатеричные символы, то второй разрыв будет
   воспринят как окончание строки октетов.
*******************************************************************************
*/
